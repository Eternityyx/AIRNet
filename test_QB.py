...
if __name__ == "__main__":
    ##### read dataset #####
    test_bs = 1    
    tmpPath = checkpoint + f"QB_17139_{condition}_best.pth"
    ReducedData = 'test_qb_multiExm1'
    ...
    FullData = 'test_qb_OrigScale_multiExm1'
    ...
    for index, data in enumerate(test_Reduced_dataloader):
        ...
        shadeOutput_np = shade.cpu().data.numpy()
        refOutput_np = ref.cpu().data.numpy()
        lms_np = data[2].numpy()
        ms_np = data[3].numpy()
        pan_np = data[1].numpy()
        gt_np = data[0].numpy()
        if reduced_count == 0:
            ...
            shade_np = np.concatenate((shadeOutput_np, shade_np), axis=0)
            ref_np = np.concatenate((refOutput_np, ref_np), axis=0)
